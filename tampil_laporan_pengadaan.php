...
<table width='100%' border='0' cellpadding='1' cellspacing='1'>
	<tr>
		<td>
			<form name='lpengadaan' method='POST' action='cetak_pengadaan.php' target='_blank'>
				<fieldset>
					<legend class='huruf'>&nbsp;<b>Form Filter Data Pengadaan</b>&nbsp;&nbsp;</legend>&nbsp;
					<table border='0'>
						<tr>
							<td>Supplier</td>
							<td>:</td>
							<td><select name='sup' class='inputbox'><option value=''>[ Pilih ]</option>
							<?php
								$sup = mysql_query ('select * from supplyer');
								while ($dsup = mysql_fetch_array ($sup)) {
									echo "<option value = '$dsup[0]'>$dsup[1] | $dsup[2]</option>";
								}
							?></select>
							</td>
						</tr>
